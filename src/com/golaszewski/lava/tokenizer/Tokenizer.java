...
public class Tokenizer {
  ...
  /**
   * Classifications for different types of characters. These will allow the
   * tokenizing state machine to make decisions on which state/action to pursue
  ...
  /**
   * Provides a base class for the processor objects - these are wrappers for a
   * processing function. The function in question resides in a state machine
   * table.
   */
  ...
  private static Map<TokenizerState, Integer> stateIndexes =
      new HashMap<TokenizerState, Integer>();

  /*
   * In order to quickly determine the next state, we construct a 2D array of
   * state transitions. To ensure easy access by indexes, the row/column
   * enumerations need to be mapped to integer indecies.
   */
  static {
  ...
  /**
   * Initializes the state table. This depends on the indexes being statically
   * configured.
   */
