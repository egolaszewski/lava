...
public class ExpressionParser {
  ...
  /**
   * Will parse a single s-expression or atom out of the token stream. This will
   * pop tokens until an s-expression or an atom has been built.
   * 
   * If an opening parenthesis is encountered for an s-expression, parsing will
   * continue until a closing parenthesis is found. If the end of input is
   * reached without encountering one, an error will be thrown.
   * 
   * @param tokenStream , the stream to pop tokens from. Only one s-expression
   *        worth of tokens will be popped off this stream per call.
   * @return an s-expression built out of tokens in the stream.
   * @throws IllegalStateException , if unexpected tokens are encountered while
   *         parsing.
   */
    ...
    switch (t.getType()) {
      ...
      case RIGHT_PARENTHESIS:
        throw new IllegalStateException(
            "Encountered unexpected right parenthesis in input.");
      default:
   ...
   * @throws IllegalStateException , if the token stream is emptied without
  ...
  private Expression parseSExpression(Queue<Token> tokenStream) {
    ...
    /*
     * Continue popping/parsing tokens and building the expression until a right
     * parenthesis is reached - the right parenthesis indicates the end of the
     * s-expression.
     */
